<template>
  <section id="about" class="relative py-24 sm:py-32 bg-gray-50/50 dark:bg-gray-900/30 overflow-hidden">
    <div class="absolute inset-0 pointer-events-none">
      <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-accent-500/[0.03] dark:bg-accent-400/[0.04] rounded-full blur-3xl" />
    </div>
    ...
    <div class="section-container relative z-10">
      ...
      <div class="mt-14 grid grid-cols-2 md:grid-cols-4 gap-4 max-w-3xl mx-auto reveal">
        <div
          v-for="item in capabilities"
          :key="item.label"
          class="group flex flex-col items-center gap-2 p-5 rounded-2xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:shadow-gray-200/40 dark:hover:shadow-black/30"
        >
          ...
          <span class="text-xs font-semibold text-gray-700 dark:text-gray-300 text-center">{{ item.label }}</span>
        </div>
      </div>
    </div>
  </section>
...
const capabilities = [
  { icon: 'lucide:rocket', label: 'Fast Delivery' },
  { icon: 'lucide:shield-check', label: 'Secure by Design' },
  { icon: 'lucide:scaling', label: 'Built to Scale' },
  { icon: 'lucide:headset', label: 'Ongoing Support' },
]
</script>
